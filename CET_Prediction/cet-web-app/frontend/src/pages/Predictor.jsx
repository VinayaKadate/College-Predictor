...
export default function Predictor() {
  ...
  // New state for PWD and Defence filters
  ...
  // Enhanced filtering with PWD and Defence quota filters
  const filteredPredictions = predictions.filter(p => {
    ...
    // Always show OPEN seats to everyone
    if (quotaCategory === 'OPEN' || quotaCategory === 'GOPENS') return true;
...
function ProfileDisplaySection({ profileData, handlePredict, loading, navigate }) {
  const isProfileComplete = profileData?.rank && profileData?.percentile && profileData?.branches?.length > 0;
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  Please complete your profile with CET rank, percentile, and preferred branches to use the predictor.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-md p-4 space-y-4">
        ...
        {(showPWD || showDefence) && (
          <div className="flex items-center gap-2 text-sm text-blue-600 bg-blue-50 px-3 py-2 rounded-lg">
            ...
            <span>
              Showing {showPWD && showDefence ? 'PWD and Defence' : showPWD ? 'PWD' : 'Defence'} quota seats along with regular seats
            </span>
          </div>
        )}
      </div>
...
function CollegeCard({ college, onAdd, isAdded }) {
  ...
  const isSpecialQuota = quotaCategory.includes('PWD') || quotaCategory.includes('DEF') || quotaCategory.includes('DEFENCE');
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-6">
        <div className="flex-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            <div className={`rounded-lg p-3 ${isSpecialQuota ? 'bg-purple-50' : 'bg-gray-50'}`}>
              <p className="text-xs text-gray-500 mb-1">Quota</p>
              <p className={`font-bold text-sm ${isSpecialQuota ? 'text-purple-700' : 'text-gray-900'}`}>
